...
function sanitizeLabel(rawLabel) {
  ...
  if (!UNEXPECTED_SCRIPT_RE.test(label)) return label;

  const digits = label.match(/[0-9０-９]+/g);
  if (!digits || !digits.length) return label; // 数字も無ければ手の施しようがないのでそのまま返す

  return `${digits.join("")}年度`;
}
...
export function normalizePopulationSeries({ fields, records }) {
  ...
  const series = records
    .map((r) => ({
      label: sanitizeLabel(r[dateField]),
