...
export async function getStaticProps() {
  ...
  return {
    props: {
      ...
      categoryData,
      categoryFieldLabel,
      chokaiTrendSeries: getChokaiHouseholdSeries(),
      chokaiTrendInsights: buildChokaiTrendInsights()
    },
    revalidate: 60 * 60 * 24
  };
}
      ...
      <section className="mx-auto max-w-5xl px-5 py-14">
        ...
        {chokaiTrendInsights ? (
          <div className="mt-10 border-t border-ink/10 pt-10">
            <SectionLabel code="FIG.T">加入世帯数の5年間の推移（船橋市統計書より）</SectionLabel>
            <p className="max-w-2xl text-sm leading-relaxed text-ink-soft">
              上記の地図・一覧は現時点のスナップショットです。経年の変化を見るために、船橋市統計書「O 市民生活」
              に収録されている過去5年分（各年4月1日現在）のデータも参考として掲載します。
            </p>
            <div className="mt-6 border border-ink/10 bg-white/60 p-5">
              <ChartErrorBoundary>
                <PopulationChart data={chokaiTrendSeries} seriesLabel="加入世帯数" unit="世帯" periodLabel="年" />
              </ChartErrorBoundary>
            </div>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-ink-soft">
              町会・自治会数は{chokaiTrendInsights.first.label}の{chokaiTrendInsights.first.count}団体から
              {chokaiTrendInsights.latest.label}の{chokaiTrendInsights.latest.count}団体へと増加した一方、
              加入世帯数は{chokaiTrendInsights.first.households.toLocaleString("ja-JP")}世帯から
              {chokaiTrendInsights.latest.households.toLocaleString("ja-JP")}世帯へと
              {chokaiTrendInsights.householdLongTermDiff < 0 ? "減少" : "増加"}しています
              （{chokaiTrendInsights.householdLongTermRate > 0 ? "+" : ""}
              {chokaiTrendInsights.householdLongTermRate.toFixed(1)}%）。団体数が増えているにもかかわらず加入世帯数が
              減っているということは、1団体あたりの平均加入世帯数が縮小していることを意味します。
            </p>
          </div>
        ) : null}
