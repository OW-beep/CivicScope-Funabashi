...
export default function LifeSanitation({
  ...
  return (
    <>
      <Seo title={`生活衛生施設 ダッシュボード｜${siteConfig.name}`} description="船橋市内の美容所・理容所・クリーニング所・旅館・ホテル・公衆浴場などの分布を地図と業種別ランキングで可視化したダッシュボードです。" path="/life-sanitation" />
      ...
      <section className="mx-auto max-w-5xl px-5 py-14">
        ...
        {error ? (
          <DataUnavailableNotice
            message={error}
            sourceUrl={datasets.lifeSanitationFacilities.sourceUrl}
            sourceLabel={datasets.lifeSanitationFacilities.label}
          />
        ) : (
          <>
